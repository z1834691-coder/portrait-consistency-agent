...
- 受邀 Streamlit 部署方向已冻结为“私有 GitHub + Community Cloud Private/受邀 Beta”；部署代码包已准备，实际创建 App、访问名单、Secrets、费用和 URL 仍需用户在控制台完成。Cloud 容器在美国且磁盘不保证持久化，不能当作生产服务器或长期数据存储。
