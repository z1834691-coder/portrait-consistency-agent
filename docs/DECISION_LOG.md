...
| D-TECH-061 | 2026-09-01 | 第一位用户入口只读检查发现侧边栏把 Cloud 页面写成“仅本机服务器”，容易让受邀测试者误解运行环境；将文案改为“运行环境：Private Demo；本机开发端口为 `127.0.0.1:8501`”。 | 已实现于本地代码；待下一次 Cloud 构建显示 | 只修正文案，不改变合同、权限、Provider、RAG、Trace 或照片数据流；避免把部署入口误写成 localhost。 | 推送后核对 Cloud 构建；真实照片流程仍由产品负责人亲自触发。 |
