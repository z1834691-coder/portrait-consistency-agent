...
| 决策 ID | 日期 | 决策 / 事实 | 状态 | 产品/工程影响 | 后续复核 |
|---|---|---|---|---|---|
| D-PROD-062 | 2026-08-30 | 为拿到可分享演示 URL，采用私有 GitHub 仓库 + Streamlit Community Cloud Private/受邀 Beta；只发布可部署代码、审核过的 Provider Card、测试和产品文档，不发布 `.env`、照片/结果图、SQLite/JSONL、模型缓存、隐藏答案或本机评测报告。 | 已冻结并实现部署包；GitHub/Cloud 控制台操作仍需用户完成 | 保持代码可迭代、可回滚和可泛化，同时避免把本机运行账本误当成云端生产存储；URL 由 Streamlit 控制台最终分配，不能预先保证某个 slug | 用户在 Cloud 选择仓库/分支/入口并配置 Secrets 后，再核验 URL、访问名单、区域、费用与真实照片授权；部署前不宣称公网服务 |
...
| D-TECH-056 | 2026-08-30 | 为 Community Cloud root entrypoint 增加 `src/` 兼容引导；移除云端不应继承的本机 bind 配置；将 `torch/transformers` 移为可选 `rag-local` extra，缺失模型时沿用 P0-A 关键词回退；`.gitignore` 明确排除本机照片、账本、报告和 hidden 材料；新增 `docs/STREAMLIT_DEPLOYMENT.md`。 | 已实现；待全量测试与私有仓库推送验证 | 同一 `app.py` 可在本机 `make run` 和 Cloud 直接执行；轻量部署不改变 RAG 只能提议和候选 Provider fail-closed 边界 | Cloud 部署后继续核对启动日志、Secrets、私有访问、资源/费用和数据出境边界 |
